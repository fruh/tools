...
function usage {
    echo "Usage: $0 [-h] script.sh target.app [icon.icns]"
    echo "Create Appname.app directory for executable."
    exit 2
}
...
TARGET=$2
ICNS_PATH=$3
ICNS=`basename "$ICNS_PATH"`
APPNAME=${2:-$(basename "$TARGET" ".app")}
...
cp "$SCRIPT" "$TARGET/Contents/MacOS/$APPNAME"
[ "$ICNS_PATH" != "" ] && cp "$ICNS_PATH" "$TARGET/Contents/Resources/"

chmod 755 "$TARGET/Contents/MacOS/$APPNAME"
